...
class Auto_Nulis_Generator {
    ...
    public function generate_article() {
        try {
            ...
            if (!$keyword) {
                $this->log_message('warning', 'No keywords available for article generation');
                return array(
                    'success' => false,
                    'message' => __('No keywords available', 'auto-nulis')
                );
            }
            
            $this->log_message('info', "Starting article generation for keyword: {$keyword}");
            
            // Check for duplicate titles
            if ($this->is_duplicate_topic($keyword)) {
                $this->log_message('info', "Skipping keyword '{$keyword}' - similar article already exists");
                return $this->generate_article(); // Try next keyword
            }
    ...
    private function log_message($level, $message, $context = array()) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Writing Agent [{$level}]: {$message}");
        }
    }
